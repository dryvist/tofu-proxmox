# homepage tag-filter local — kept out of locals.tf so that file stays under the
...
locals {
  # Homepage (gethomepage) — service-launcher dashboard, web UI on homepage_web (3000), config
  ...
  homepage_container_ids = {
    for k, v in var.containers : k => v.vm_id
    if contains(coalesce(try(v.tags, null), []), "homepage")
  }
}
