...
variable "containers" {
  ...
  type = map(object({
    ...
    # OS template override: a bare vztmpl filename on var.datastore_iso. Null
    # = the estate's shared Debian template (there was no per-guest selector
    # at all before NixOS guests). Set with os_type = "unmanaged" for NixOS —
    # Proxmox has no nixos ostype, and Debian's hooks rewrite /etc files the
    # guest owns declaratively. SET-ONCE: template_file_id is in the container
    # module's ignore_changes. See ct_templates.tf.
    ct_template = optional(string)

    # Proxmox HA opt-in; default false. Enrolling also accepts the relocation
    # hazard imports.tf documents. See ha.tf.
    ha = optional(bool, false)

    # Negative-affinity group: members are kept on separate nodes by a
