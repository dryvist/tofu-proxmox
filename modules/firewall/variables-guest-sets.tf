# Guest-set inputs for the firewall module: one map per tag-derived guest set,
# populated by the caller from the tag filters in modules/proxmox-stack. Every
# rule file selects the guests it applies to from one of these maps, so an empty
# map means the rules it feeds simply do not render.
#
# Split out of variables.tf (locals and variables merge across files in a
# module) so that file stays under the shared _file-size 12 KB error threshold.
