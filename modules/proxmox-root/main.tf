...
# This module carries the composition that used to live at the repository
# root (providers, ephemeral credential fetches, the deployment-object
# decode, and the call into modules/proxmox-stack) so it can be sourced by
# another repository's own root module (Vikunja 3492) — a caller such as a
# git-tracked desired-state repo cannot source this repository's actual root
# directly, because OpenTofu permits a `cloud` block and `import` blocks only
# in the true root module, and this repository's root has both (its own
# `cloud` workspace binding, and the adoption imports in ../imports.tf).
#
# Every existing infrastructure address is preserved: the repository's own
# root now calls this module as `module.stack`, with a `moved` block in
# ../migrations.tf recording the one-level reparenting of `module.homelab`.
# This mirrors the same technique that file already uses for the prior
# root-wrapping migration.
