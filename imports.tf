...
# NOTE: the win11-vdi answer-file ISO deliberately has no import block and no
# resource. Unlike the install ISOs above, its object is generated by
# scripts/windows-vdi/build-win11-answer-iso.sh and did not exist yet, so the
# import failed the PLAN outright ("Cannot import non-existent remote object")
# and took every unrelated resource in the workspace down with it. An import is
# an adoption of something already there; it cannot stand in for a create.
#
# Re-add the resource and this import together, in the same change that first
# places the object. Note also that the build script writes to a different node
# than local.deployment.proxmox_node, which the import id is derived from —
# reconcile those two before re-adding, or the import fails again with the file
# present but on the wrong node.
