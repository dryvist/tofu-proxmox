...
### Interactive and autonomous remote plan
...
> may run `tofu init` and `plan` autonomously for the duration of the session,
> relying on the pre-existing `tofu login` token. **`apply` is not in that
> set** — it is refused server-side (`allowRemoteApply = false`) and belongs to
> a Terrakube job, which is a deliberate, audited action rather than a step in
> an autonomous loop.
